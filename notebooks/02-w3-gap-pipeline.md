...
- [x] 跨 input majority vote 合并（`--merge-inputs` 选项）
- [x] asm context 集成
- [x] Train/test 划分（4 kernel train / 2 kernel test）
- [x] Schema 验证 + 人工抽查
- [x] **P0 扩展**：默认 per-(kernel, input, PC) 模式，345 样本（见下）
...
| `data/dataset/train.jsonl` | 新增 | 172 条训练样本 |
| `data/dataset/test.jsonl` | 新增 | 173 条测试样本 |

---

# W3b Step 6 P0 扩展 — 不做跨 input 合并

> 完成：2026/05/15
> 触发：原始合并版本只有 177 条，远低于 5K+ 目标；先做 P0 扩展（无新计算）

## 改动

`scripts/build_dataset.py` 默认行为改为 **per-(kernel, input, PC)** 出独立样本；
新增 `--merge-inputs` flag 保留原合并行为。

| 模式 | 样本数 | Train | Test |
|---|---:|---:|---:|
| `--merge-inputs`（原行为） | 177 | 94 | 83 |
| 默认（per-trace） | **345** | **172** | **173** |

## 设计依据

合并版本假设"同 binary 同 PC 的 label 跨 input 应该一致"，但实测：

- 多 input 出现的 PC：56/70（train）
- 这些 PC 中 **label 跨 input 发散的有 22 个（39%）**

→ 说明对同一 PC，**不同输入特征（图大小/稀疏度）会导致最优 prefetcher 不同**。
将跨 input 的样本作为独立训练数据，模型可以学到 input-dependent prefetching 决策。
...
| 划分 | 记录数 | Kernels | 文件大小 |
|---|---:|---|---:|
| Train | 172 | bfs(56), bc(41), pr(41), cc(34) | 2.2 MB |
| Test | 173 | sssp(104), tc(69) | 2.3 MB |
| **合计** | **345** | 6 kernel × ≤4 input | 4.5 MB |

### Label 分布

| 维度 | Train | Test |
|---|---|---|
| Filter=True | 70/172 (40.7%) | 81/173 (46.8%) |
| PF Selection | sandbox(79), stream(11), ip_stride(7), sms(5) | sandbox(45), stream(18), ip_stride(14), sms(15) |
| PF Degree | d1(29), d2(19), d3(54) | d1(32), d2(20), d3(40) |

注：test 中 SMS 占比（15/173 = 8.7%）比 train（5/172 = 2.9%）高，因为 sssp/tc
工作负载有 SMS 友好的 spatial pattern。这个分布偏差是真实数据特征，不是 bug。

### 替代 min-count 选项

| min-count | 总样本 | 备注 |
|---:|---:|---|
| 1 | **2288** | 单次 fill AMAT 噪声大，label 不稳 |
| 2 | 579 | 中等可靠性 |
| **3**（默认） | **345** | 统计可靠性与样本量平衡 |
| 5 | 398 | 略保守 |
...
| 条件 | 状态 | 备注 |
|---|---|---|
| 行数 5K+ | ❌ 345 行 | 仍未达成；需要 P1（libc objdump）+ P2（更长 cap） |
| label 分布合理 | ⚠️ Filter 40-47%，SMS 仍偏低 | 略好于合并版（37-45%） |
| 每行 schema 验证通过 | ✅ | |
| 抽样验证 | ✅ | label-AMAT 自洽 |

## P0 完成度

- [x] `build_dataset.py` 默认改为 per-trace 模式
- [x] `--merge-inputs` flag 保留原合并行为
- [x] 记录中加 `input` 字段（合并模式下为 null）
- [x] 多 input 发散 label 的真实存在验证（39% divergent）
- [x] 样本量 177 → 345（+95%）
...
| 优先级 | 方案 | 预估新增 | 代价 |
|---|---|---:|---|
| P1 | libc.so objdump（覆盖原本被跳过的 ~80% PC） | +2000-3000 | 半天 |
| P2 | Cap 50M → 200M 重跑 trace + grid | min-count 过滤丢弃更少 | ~半天 |
| - | 直接进 W4 用 345 条跑 pipeline 验证 | 不增加数据 | 工程优先 |
