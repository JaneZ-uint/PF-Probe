...
#
# Usage: scripts/run_w3_grid.sh [parallelism] [sim_M] [warmup_M] [inputs_csv] [kernels_csv] [out_dir]
#
...
SIM_M="${2:-30}"
WARMUP_M="${3:-1}"
INPUTS_CSV="${4:-kron18,kron20,urand18,urand20}"
KERNELS_CSV="${5:-bfs,pr,sssp,bc,cc,tc}"
OUT_DIR="${6:-${REPO_ROOT}/data/w3_grid}"
...
EXPECTED=$((${#KERNELS[@]} * ${#INPUTS[@]} * ${#CONFIGS[@]}))
echo
echo "========================================="
echo "Grid complete: ${DONE}/${EXPECTED} requested JSONs in ${ELAPSED}s ($(( ELAPSED / 60 ))m$(( ELAPSED % 60 ))s)"
echo "========================================="
